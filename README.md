...
**목표**: 문법 규칙 학습, 기초 지식 Q&A, Teacher 도입  
**핵심**: Ollama Teacher가 처음 등장하는 단계

```
데이터 구성 (~80,000건 | max_seq_len: 512):
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Korean Wikipedia (200~500자)               주류  ~80,000건
    - 이해 가능한 지식 수준의 단락
    - split: train[:5,000]
  
  Teacher Q&A 합성 데이터 (캐시 활용)         소량  ~450건
    - Wikipedia 150개 문서 → 초등 수준 Q&A 3쌍/문서
    - generate_qa_pairs(passage, stage=2, n=3)
    - 형식: "Q: 광합성이란?\nA: 식물이 햇빛으로 음식을 만드는 과정이에요."
    - 병렬: ThreadPoolExecutor(max_workers=2)
    - 캐시: data/teacher_cache/ (MD5 해시)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Teacher 설정:
  model: gemma4:e4b (또는 llama3.2:3b)
  timeout: 240초 (로컬 Ollama 과부하 방지)
  cache: MD5 해시 기반 — 재실행 시 즉시 반환

하이퍼파라미터:
  lr=5e-5, warmup=300, batch=4, grad_accum=4
  EWC λ=3000
  replay=15% (Stage 0~1 누적)
...
**목표**: 단계적 논리 추론, Chain-of-Thought 학습  
**핵심**: Teacher의 단계별 풀이(CoT)가 핵심 데이터

```
데이터 구성 (~100,000건 | max_seq_len: 512):
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Korean Wikipedia (250~600자)              ~50%  ~50,000건
    - 논리적 설명이 포함된 단락
    - split: train[:8,000]
  
  Teacher CoT 풀이 (핵심 ⭐)              ~30%  ~30,000건
    수학 문제 (10개):
      - "2x + 5 = 13일 때 x를 구하세요."
      - "삼각형의 넓이: 밑변 6, 높이 4"
      - "1부터 100까지의 합"
    ...
    논리 문제 (5개, 영문):
      - "All cats are animals... Do cats breathe?"
      - "3L/5L 저그로 4L 측정하기"
      - 수열 패턴 추론 등
    
    → 각 문제 × 8회 반복 = ~184 × 8 = 1,472개 CoT 샘플
  ...
  Ollama 합성 데이터                        ~5%  ~5,000건
  Stage 0~2 Replay                         ~15%  ~15,000건
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
하이퍼파라미터:
  lr=3e-5, warmup=300, batch=4, grad_accum=4
  EWC λ=2000
  replay=10%
