"""EurekaAI — Stage 6 (Social) Training. 졸업 기준: PPL ≤ 5 (최고 난이도)"""
...
PREV_CKPT = Path("checkpoints/stage5_university/stage5_university/best")

def setup_logging(prefix="stage6_social"):
...
def load_model(config):
    ...
    if PREV_CKPT.exists():
        ...
        logger.info(f"✅ Stage 5 체크포인트 로드: {PREV_CKPT}")
    else:
...
def build_dataloaders(config, tokenizer):
    ...
    if not Path(train_path).exists():
        from stages.stage6_social.data_prep import main as prep; prep()
...
def main(config_path=None, reset=False):
    logger.info("=" * 55)
    logger.info("   🌐 EurekaAI — Stage 6: Social Training (FINAL)")
    logger.info("=" * 55)
    ...
    graduated = ppl <= 5.0
    logger.info(f"📊 Stage 6 최종 PPL: {ppl:.2f} ({'🎉 CURRICULUM COMPLETE!' if graduated else '❌ 미졸업 (기준 PPL≤5)'})")
    progression.record_eval(6, {"ppl": ppl}, step=config.max_steps)
    if graduated:
        progression.complete_stage(6, checkpoint_path="checkpoints/stage6_social/best")
        logger.info("🎊🎊🎊 EurekaAI 전체 커리큘럼 완료! 신생아 → 사회인 성장 달성! 🎊🎊🎊")
    model.save_pretrained("checkpoints/stage6_social/final_model")
    progression.print_summary()
    logger.info("✅ Stage 6 학습 완료!")
    return graduated
