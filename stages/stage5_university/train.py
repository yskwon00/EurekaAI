"""EurekaAI — Stage 5 (University) Training. 졸업 기준: PPL ≤ 6"""
...
PREV_CKPT = Path("checkpoints/stage4_high/stage4_high/best")

def setup_logging(prefix="stage5_university"):
...
def load_model(config):
    ...
    if PREV_CKPT.exists():
        ...
        logger.info(f"✅ Stage 4 체크포인트 로드: {PREV_CKPT}")
    else:
...
def build_dataloaders(config, tokenizer):
    ...
    if not Path(train_path).exists():
        from stages.stage5_university.data_prep import main as prep; prep()
...
def main(config_path=None, reset=False):
    logger.info("=" * 55)
    logger.info("   🎓 EurekaAI — Stage 5: University Training")
    logger.info("=" * 55)
    ...
    graduated = ppl <= 6.0
    logger.info(f"📊 Stage 5 최종 PPL: {ppl:.2f} ({'✅ GRADUATED!' if graduated else '❌ 미졸업 (기준 PPL≤6)'})")
    progression.record_eval(5, {"ppl": ppl}, step=config.max_steps)
    if graduated:
        progression.complete_stage(5, checkpoint_path="checkpoints/stage5_university/best")
    model.save_pretrained("checkpoints/stage5_university/final_model")
    progression.print_summary()
    logger.info("✅ Stage 5 학습 완료!")
    return graduated
